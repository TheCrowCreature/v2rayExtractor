...
# تنظیمات اصلی
GITHUB_REPOSITORY = "TheCrowCreature/v2rayExtractor"
GITHUB_REPO_URL = f"https://github.com/{GITHUB_REPOSITORY}"

# لینک‌های دعوت اختصاصی شما
MAIN_INVITE_LINK = "https://t.me/+hujZl_lRBv0wZmEx"
CONFIG_INVITE_LINK = "https://t.me/+mLeDtu0K_qEzZmVk"
...
def regroup_configs_by_source(checked_configs: List[str]) -> Dict[str, List[str]]:
    ...
    for config in checked_configs:
        ...
        try:
            ...
            logging.warning(f"Could not parse tag for source detection: {e}")
...
def send_all_grouped_configs(bot: telebot.TeleBot, channel_id: str, grouped_configs: Dict[str, List[str]]):
    for source in sorted(grouped_configs.keys()):
        ...
        logging.info(f"Sending {len(configs)} configs from source '{source}'")
        ...
        for i in range(0, len(configs), 15):
            ...
            iran_tz = pytz.timezone("Asia/Tehran")
            time_ir = datetime.now(iran_tz).strftime("%H:%M:%S")
            from_link = f"https://t.me/{source[1:]}" if source.startswith('@') else "#"

            # کپشن تمیز با لینک‌های دعوت شما
            caption = (
                f"{message_text}\n\n"
                f"⏰ **Time:** {time_ir}\n"
                f"🔍 **From:** [{source}]({from_link})\n\n"
                f"🆔 **Main Channel:** [Join Here]({MAIN_INVITE_LINK})\n"
                f"🚀 **Config Channel:** [Join Here]({CONFIG_INVITE_LINK})"
            )
